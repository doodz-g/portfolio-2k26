...
export function Header() {
  ...
  return <header className="fixed inset-x-0 top-0 z-50 border-b border-[var(--line)] bg-[rgba(245,243,235,.94)] backdrop-blur-xl">
    <div className="container flex h-[76px] items-center justify-between">
      <a href="#top" className="focus-ring display text-xl font-extrabold" aria-label="Eduvigis Garcia home">EG<span className="text-[var(--orange)]">.</span></a>
      <nav className="hidden items-center gap-5 lg:flex" aria-label="Main navigation">{links.map(([label,id])=><a key={id} href={`#${id}`} aria-current={active===id?"location":undefined} className={`nav-link ${active===id?"nav-link-active":""}`}>{label}</a>)}</nav>
      <a href="#contact" className="button button-primary hidden !min-h-10 !px-4 text-sm lg:inline-flex">Let&apos;s talk</a>
    ...
    {open&&<nav id="mobile-menu" className="container grid max-h-[calc(100vh-76px)] grid-cols-2 gap-1 overflow-auto border-t border-[var(--line)] py-4 lg:hidden" aria-label="Mobile navigation">{links.map(([label,id])=><a key={id} href={`#${id}`} onClick={()=>setOpen(false)} className="focus-ring rounded-xl px-3 py-3 text-base font-bold hover:bg-white/60">{label}</a>)}</nav>}
  </header>;
}
